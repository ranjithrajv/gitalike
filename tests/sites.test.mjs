...
describe('hostsFor', () => {
  ...
  test('never duplicates a host and never mixes kinds', () => {
    ...
    assert.deepEqual(
      ...
      ['codeberg.org', 'gitea.com', 'github.com', 'gh.acme.com'],
    );
  });
  ...
  test('ignores stored keys that are not bare hostnames', () => {
    ...
    assert.deepEqual(
      ...
      ['codeberg.org', 'gitea.com', 'github.com', 'ok.example'],
    );
  });
});
...
describe('manifest permissions', () => {
  test('the manifest host_permissions are derived from the bundled hosts', () => {
    // `build.mjs` writes them from the registry (`hostPermissions`), so the
    // manifest and the picker cannot drift; the base manifest must not repeat
    // the list. Everything else is requested one origin at a time from the popup.
    ...
    assert.equal(
      ...
      undefined,
      'host_permissions are generated by build.mjs, not listed in the base',
    );
    const expected = Object.keys(kinds)
      .flatMap((kind) => hostsFor(kind, {}))
      .map((host) => `*://${host}/*`);
    assert.deepEqual([...hostPermissions()].sort(), expected.sort());
    assert.match(
      readFileSync(new URL('../build.mjs', import.meta.url), 'utf8'),
      /host_permissions: hostPermissions\(\)/,
    );
    assert.ok(
      (base.optional_host_permissions || []).length > 0,
      'self-hosted origins are optional, not granted at install',
    );
    assert.ok(
      !hostPermissions().some((p) => /^\*:\/\/\*\/\*$/.test(p)),
